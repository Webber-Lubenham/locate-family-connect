...
import { RefreshCw, Share, Map, Layers } from 'lucide-react';
...
const MAP_STYLES = {
  ...
  hybrid: 'mapbox://styles/mapbox/satellite-v9',  // Imagem de satélite com estradas
  light: 'mapbox://styles/mapbox/light-v11',
  dark: 'mapbox://styles/mapbox/dark-v11',
  navigation: 'mapbox://styles/mapbox/navigation-day-v1'
};

// Estilo padrão é híbrido ou o definido no env
const DEFAULT_STYLE = env.MAPBOX_STYLE_URL || MAP_STYLES.hybrid;
...
const StudentLocationMap: React.FC<StudentLocationMapProps> = ({
    ...
    if (map.current && mapLoaded) {
      ...
      const onStyleLoad = () => {
        if (currentLocation && map.current) {
          ...
          updateMapWithPosition({ coords: { latitude, longitude, accuracy: 10 } } as GeolocationPosition, false);
        }
      };
  ...
  const startAutoUpdate = () => {
    ...
    const intervalId = setInterval(() => {
      if (!realTimeActive) { // Só atualiza pelo intervalo se o tempo real não estiver ativo
        handleUpdateLocation();
      }
      ...
      watchPositionId.current = navigator.geolocation.watchPosition(
        ...
        (error) => {
          ...
          safeToast({
            ...
            description: error.message || "Verifique as permissões de localização",
            variant: "destructive"
          });
  ...
  // Função comum para atualizar o mapa com uma posição
  const updateMapWithPosition = (position: GeolocationPosition, showNotification: boolean = true) => {
  ...
  const MapStyleSelector = () => (
    ...
    >
      <div className="p-2">
        <h3 className="text-sm font-medium mb-2 text-gray-700">Estilo do Mapa</h3>
        <div className="grid grid-cols-2 gap-2">
          {(Object.keys(MAP_STYLES) as Array<keyof typeof MAP_STYLES>).map((styleName) => (
            <button
              key={styleName}
        ...
        <div className="flex flex-col relative">
          ...
          <div className={`flex items-center justify-between bg-slate-100 p-2 ${getStatusIndicatorSize()}`}>
            <div className="flex items-center">
              <div className={`h-2 w-2 rounded-full ${autoUpdateActive ? 'bg-green-500' : realTimeActive ? 'bg-blue-500' : 'bg-amber-500'} mr-2`}></div>
              <span className="text-gray-700">
                {realTimeActive ? 'Tempo real ativo' : autoUpdateActive ? 'Atualização automática ativa' : 'Atualização manual'}
              </span>
            </div>
            {lastUpdateTime && (
