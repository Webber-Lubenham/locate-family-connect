...
describe('Compartilhamento de Localização', () => {
  ...
  it('renderiza clusters e marcadores individuais no mapa', () => {
    // Aguardar o mapa carregar
    cy.get('[data-cy="location-map-container"]', { timeout: 15000 }).should('exist');
    // Verificar se pelo menos um cluster ou marcador individual está presente
    cy.get('body').then(($body) => {
      const hasCluster = $body.find('[data-cy="cluster-marker"]').length > 0;
      const hasSingle = $body.find('[data-cy="single-marker"]').length > 0;
      expect(hasCluster || hasSingle).to.be.true;
    });
    // Se clusters existirem, clicar no primeiro para testar zoom
    cy.get('[data-cy="cluster-marker"]').first().click({ force: true });
    cy.log('✅ Clusters e marcadores individuais renderizados e interativos');
  });
});
